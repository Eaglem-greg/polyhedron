...
class TestR3(unittest.TestCase):
    ...
    def test_length0(self):
        unit_x = R3(1.0, 0.0, 0.0)
        unit_y = R3(0.0, 1.0, 0.0)
        unit_z = R3(0.0, 0.0, 1.0)
        self.assertEqual(unit_x.length(), 1.0)
        self.assertEqual(unit_y.length(), 1.0)
        self.assertEqual(unit_z.length(), 1.0)
